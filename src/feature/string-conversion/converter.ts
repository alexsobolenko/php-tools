...
import App from '../../app';
import {M_INFO, S_CONCATENATION, S_INTERPOLATION, S_SPRINTF} from '../../constants';
import {IPhpNode, nodeName, tryParsePhp, walkPhp} from '../../utils/php-ast';
...
export default class StringConverter {
    ...
    public render() {
        ...
        if (template === '' || this.range === null) {
            ...
            App.instance.showMessage('No string expression found to convert, or the expression already does not need conversion.', M_INFO);
    ...
    private template(): string {
        ...
        let template = '';
        if (this.target === S_CONCATENATION) {
            template = this.concatenationTemplate(parts);
        } else if (this.target === S_INTERPOLATION) {
            template = this.interpolationTemplate(parts);
        } else if (this.target === S_SPRINTF) {
    ...
    private findSource(document: TextDocument, position: Position): {type: string, node: IPhpNode}|null {
        ...
        walkPhp(program, (node, parent) => {
            ...
            if (this.isConcat(node) && !this.isConcat(parent)) {
                candidates.push({type: S_CONCATENATION, node});

                return;
            }

            if (this.isSprintf(node)) {
                candidates.push({type: S_SPRINTF, node});

                return;
            }

            if ((node as IPhpNode).kind === 'encapsed') {
                candidates.push({type: S_INTERPOLATION, node});
            }
        });
    ...
    private collectParts(document: TextDocument, type: string, node: IPhpNode): Array<any>|null {
        if (type === S_CONCATENATION) {
            return this.collectConcatParts(document, node);
        }

        if (type === S_INTERPOLATION) {
